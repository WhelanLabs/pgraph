...
public class AQLActorsAndMoviesExample {
   ...
   public static void setUp() throws InterruptedException, ExecutionException {
      arangoDB = new ArangoDBAsync.Builder().user("root").password("openSesame").serializer(new ArangoJack()).build();
   ...
   private static DocumentCreateEntity<BaseDocument> saveMovie(final ArangoCollectionAsync movies, final String key, final String title,
         final int released, final String tagline) throws InterruptedException, ExecutionException {
   ...
   private static DocumentCreateEntity<BaseDocument> saveActor(final ArangoCollectionAsync actors, final String key, final String name,
   ...
   private static void saveActsIn(final ArangoCollectionAsync actsIn, final String actor, final String movie, final String[] roles, final int year)
   ...
   private static void createData() throws InterruptedException, ExecutionException {
      ...
      final String theMatrix = saveMovie(movies, "TheMatrix", "The Matrix", 1999, "Welcome to the Real World").getId();
      ...
      final String theMatrixReloaded = saveMovie(movies, "TheMatrixReloaded", "The Matrix Reloaded", 2003, "Free your mind").getId();
      ...
      final String theDevilsAdvocate = saveMovie(movies, "TheDevilsAdvocate", "The Devil's Advocate", 1997, "Evil has its winning ways").getId();
      ...
      final String topGun = saveMovie(movies, "TopGun", "Top Gun", 1986, "I feel the need, the need for speed.").getId();
      ...
      final String jerryMaguire = saveMovie(movies, "JerryMaguire", "Jerry Maguire", 2000, "The rest of his life begins now.").getId();
      ...
      final String snowFallingonCedars = saveMovie(movies, "SnowFallingonCedars", "Snow Falling on Cedars", 1999, "First loves last. Forever.")
            .getId();
      ...
      final String youveGotMail = saveMovie(movies, "YouveGotMail", "You've Got Mail", 1998, "At odds in life... in love on-line.").getId();
      ...
      final String sleeplessInSeattle = saveMovie(movies, "SleeplessInSeattle", "Sleepless in Seattle", 1993,
            "What if someone you never met, someone you never saw, someone you never knew was the only someone for you?").getId();
      ...
      saveActsIn(actsIn, megR, joeVersustheVolcano, new String[] { "DeDe', 'Angelica Graynamore', 'Patricia Graynamore" }, 1990);
      saveActsIn(actsIn, nathan, joeVersustheVolcano, new String[] { "Baw" }, 1990);

      final String whenHarryMetSally = saveMovie(movies, "WhenHarryMetSally", "When Harry Met Sally", 1998, "At odds in life... in love on-line.")
            .getId();
   ...
   public void allActorsActsInMovie1or2() throws InterruptedException, ExecutionException {
      final CompletableFuture<ArangoCursorAsync<String>> f = db.query(
            "WITH actors FOR x IN ANY 'movies/TheMatrix' actsIn OPTIONS {bfs: true, uniqueVertices: 'global'} RETURN x._id", null, null,
            String.class);
   ...
   public void allActorsActsInMovie1or2UnionDistinct() throws InterruptedException, ExecutionException {
      ...
      f.whenComplete((cursor, ex) -> assertThat(cursor.asListRemaining(),
            hasItems("actors/Emil", "actors/Hugo", "actors/Carrie", "actors/Laurence", "actors/Keanu", "actors/Al", "actors/Charlize"))).get();
   }
   ...
   public void allActorsWhoActedIn3orMoreMovies() throws InterruptedException, ExecutionException {
      final CompletableFuture<ArangoCursorAsync<Actor>> f = db.query(
            "FOR x IN actsIn COLLECT actor = x._from WITH COUNT INTO counter FILTER counter >= 3 RETURN {actor: actor, movies: counter}", null, null,
            Actor.class);
      f.whenComplete(
            (cursor, ex) -> assertThat(cursor.asListRemaining(),
                  hasItems(new Actor("actors/Carrie", 3), new Actor("actors/CubaG", 4), new Actor("actors/Hugo", 3), new Actor("actors/Keanu", 4),
                        new Actor("actors/Laurence", 3), new Actor("actors/MegR", 5), new Actor("actors/TomC", 3), new Actor("actors/TomH", 3))))
            .get();
   }
   ...
   public void allMoviesWhereExactly6ActorsActedIn() throws InterruptedException, ExecutionException {
      final CompletableFuture<ArangoCursorAsync<String>> f = db
            .query("FOR x IN actsIn COLLECT movie = x._to WITH COUNT INTO counter FILTER counter == 6 RETURN movie", null, null, String.class);
      f.whenComplete(
            (cursor, ex) -> assertThat(cursor.asListRemaining(), hasItems("movies/SleeplessInSeattle", "movies/TopGun", "movies/YouveGotMail")))
            .get();
   }
   ...
   public void theNumberOfActorsByMovie() throws InterruptedException, ExecutionException {
      final CompletableFuture<ArangoCursorAsync<Movie>> f = db
            .query("FOR x IN actsIn COLLECT movie = x._to WITH COUNT INTO counter RETURN {movie: movie, actors: counter}", null, null, Movie.class);
      f.whenComplete((cursor, ex) -> assertThat(cursor.asListRemaining(),
            hasItems(new Movie("movies/AFewGoodMen", 11), new Movie("movies/AsGoodAsItGets", 4), new Movie("movies/JerryMaguire", 9),
                  new Movie("movies/JoeVersustheVolcano", 3), new Movie("movies/SleeplessInSeattle", 6), new Movie("movies/SnowFallingonCedars", 4),
                  new Movie("movies/StandByMe", 7), new Movie("movies/TheDevilsAdvocate", 3), new Movie("movies/TheMatrix", 5),
                  new Movie("movies/TheMatrixReloaded", 4), new Movie("movies/TheMatrixRevolutions", 4), new Movie("movies/TopGun", 6),
                  new Movie("movies/WhatDreamsMayCome", 5), new Movie("movies/WhenHarryMetSally", 4), new Movie("movies/YouveGotMail", 6))))
            .get();
   }
   ...
   public void theNumberOfMoviesByActor() throws InterruptedException, ExecutionException {
      final CompletableFuture<ArangoCursorAsync<Actor>> f = db
            .query("FOR x IN actsIn COLLECT actor = x._from WITH COUNT INTO counter RETURN {actor: actor, movies: counter}", null, null, Actor.class);
      f.whenComplete((cursor, ex) -> assertThat(cursor.asListRemaining(),
            hasItems(new Actor("actors/Al", 1), new Actor("actors/AnnabellaS", 1), new Actor("actors/AnthonyE", 1), new Actor("actors/BillPull", 1),
                  new Actor("actors/BillyC", 1), new Actor("actors/BonnieH", 1), new Actor("actors/BrunoK", 1), new Actor("actors/Carrie", 3),
                  new Actor("actors/CarrieF", 1), new Actor("actors/Charlize", 1), new Actor("actors/ChristopherG", 1), new Actor("actors/CoreyF", 1),
                  new Actor("actors/CubaG", 4), new Actor("actors/DaveC", 1), new Actor("actors/DemiM", 1), new Actor("actors/Emil", 1),
                  new Actor("actors/EthanH", 1), new Actor("actors/GregK", 2), new Actor("actors/HelenH", 1), new Actor("actors/Hugo", 3),
                  new Actor("actors/JackN", 2), new Actor("actors/JamesC", 1), new Actor("actors/JamesM", 1), new Actor("actors/JayM", 1),
                  new Actor("actors/JerryO", 2), new Actor("actors/JohnC", 1), new Actor("actors/JonathanL", 1), new Actor("actors/JTW", 1),
                  new Actor("actors/Keanu", 4), new Actor("actors/KellyM", 1), new Actor("actors/KellyP", 1), new Actor("actors/KevinB", 1),
                  new Actor("actors/KevinP", 1), new Actor("actors/KieferS", 2), new Actor("actors/Laurence", 3), new Actor("actors/MarshallB", 1),
                  new Actor("actors/MaxS", 2), new Actor("actors/MegR", 5), new Actor("actors/Nathan", 1), new Actor("actors/NoahW", 1),
                  new Actor("actors/ParkerP", 1), new Actor("actors/ReginaK", 1), new Actor("actors/ReneeZ", 1), new Actor("actors/RickY", 1),
                  new Actor("actors/RitaW", 1), new Actor("actors/RiverP", 1), new Actor("actors/Robin", 1), new Actor("actors/RosieO", 1),
                  new Actor("actors/SteveZ", 1), new Actor("actors/TomC", 3), new Actor("actors/TomH", 3), new Actor("actors/TomS", 1),
                  new Actor("actors/ValK", 1), new Actor("actors/VictorG", 1), new Actor("actors/WernerH", 1), new Actor("actors/WilW", 1))))
            .get();
   }
   ...
   public void theNumberOfMoviesActedInBetween2005and2010byActor() throws InterruptedException, ExecutionException {
      ...
      f.whenComplete((cursor, ex) -> assertThat(cursor.asListRemaining(),
            hasItems(new Actor("actors/BillPull", 1), new Actor("actors/ChristopherG", 1), new Actor("actors/CubaG", 1), new Actor("actors/DemiM", 1),
                  new Actor("actors/JackN", 1), new Actor("actors/JamesM", 1), new Actor("actors/JTW", 1), new Actor("actors/KevinB", 1),
                  new Actor("actors/KieferS", 1), new Actor("actors/MegR", 2), new Actor("actors/Nathan", 1), new Actor("actors/NoahW", 1),
                  new Actor("actors/RitaW", 1), new Actor("actors/RosieO", 1), new Actor("actors/TomC", 1), new Actor("actors/TomH", 2),
                  new Actor("actors/VictorG", 1))))
            .get();
   }
   ...
   public static class Actor {
      ...
      public boolean equals(final Object obj) {
         ...
         if (movies == null) {
            ...
            return movies.equals(other.movies);
      }
