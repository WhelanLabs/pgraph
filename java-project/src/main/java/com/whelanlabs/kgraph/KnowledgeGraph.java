...
public class KnowledgeGraph {
   ...
   private static ArangoDB _db = null;
   ...
   private static ArangoDB getDB() {
      if (null == _db) {
         _db = new ArangoDB.Builder().user("root").password("openSesame").serializer(new ArangoJack()).build();
      }
      return _db;
   }
   ...
   public Collection<CollectionEntity> getCollections() {
      arangoDB = getDB();
      DbName dbName = DbName.normalize(_db_name);
      return arangoDB.db(dbName).getCollections();
   }
}
